...
passed the live-Blue inject into produce_checkpoint WITHOUT re-passing shaw_attached_fn — so
...
precondition: re-pass shaw_attached_fn down so produce_checkpoint RE-CHECKS just before
injecting (accepting a 2nd tmux list-clients).

Drives the REAL produce_checkpoint against a REAL WalStore with a flip-flop shaw_attached_fn
...
def test_shaw_attaching_between_wrapper_check_and_inject_is_recaught(tmp_path):
    ...
    out = cp.produce_checkpoint_supervised(
        ...
        shaw_attached_fn=shaw_attached_fn,  # flip-flop: False then True
        resolve_body=resolver, now=_fake_clock(), sleep=lambda s: None)

    assert calls["operator"] >= 2, "produce_checkpoint must RE-CHECK shaw just before injecting"
...
def test_no_shaw_still_injects_and_blue_authors(tmp_path):
    ...
    out = cp.produce_checkpoint_supervised(
        ...
        shaw_attached_fn=lambda: False,     # absent throughout
        resolve_body=resolver, now=_fake_clock(), sleep=lambda s: None)
