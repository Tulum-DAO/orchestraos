...
# ---- item C: arturo:local-stt + dashboard:https rows -----------------------------------------------
def _stt_probes(state_json, **kw):
    p = _probes(**kw)
    real = p.run_cmd
    def run_cmd(argv):
        return state_json if any("local_stt" in a for a in argv) else real(argv)
    return D.DoctorProbes(**{**p.__dict__, "run_cmd": run_cmd})
...
def test_dashboard_https_note_only_off_loopback(tmp_path):
    root = _repo(tmp_path)
    st = S.load_settings(repo_root=root, config_path=root / "orchestra.toml")
    assert "dashboard:https" not in _by_name(D.run_doctor(st, _probes()))
    st2 = S.load_settings(repo_root=root, config_path=root / "orchestra.toml")
