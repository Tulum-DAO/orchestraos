...
3. **Spawn dev agents** when needed: `~/scripts/agent-orchestra/spawn-agent.sh {agent-id} --task "description"`
...
2. For each subtask, either:
   ...
   - Spawn a new dev agent: `~/scripts/agent-orchestra/spawn-agent.sh {agent-id} --task "description"`
...
1. Create a questionnaire HTML file in `~/scripts/agent-orchestra/questionnaires/`
...
- `~/scripts/agent-orchestra/state/clients/{CLIENT_SLUG}/` — client records
...
1. Create a staging environment: `python3 ~/scripts/agent-orchestra/scripts/staging-env.py create {PROJECT} --branch {BRANCH}`
